...
object Spark05_SparkSQL_Test {
  def main(args: Array[String]): Unit = {
    ...
    spark.sql(
      """
        |load data local inpath 'datas/product_info.txt' into table exer.product_info
        |""".stripMargin)
    ...
    spark.sql(
      """
        |load data local inpath 'datas/city_info.txt' into table exer.city_info
        |""".stripMargin)
